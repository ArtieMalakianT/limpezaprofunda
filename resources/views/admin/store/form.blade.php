<x-app2-layout>
    @push('title')
        <h3>Editar Copy Loja Virtual</h3>
    @endpush
                ...
                <form action="{{Route('storeUpdate')}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            <label>Copy</label>                            
                            <textarea class="form-control" id="summernote" name="copy"><?php $file = fopen(public_path($store->copy),'r'); while(!feof($file)){$linha = fgets($file,'1024'); echo $linha;} fclose($file); ?></textarea>
                            @error('copy')
                        ...
                        <div class="form-group">
                            <label>Link</label>
                            <input class="form-control" type="text" name="link" value="{{$store->link}}" />
                            @error('link')
                                <div class="aler alert-danger">{{$message}}</div>
                            @enderror
                        </div>
